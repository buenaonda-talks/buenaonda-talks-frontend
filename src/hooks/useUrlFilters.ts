...
type FilterParamType = 'multiple-string' | 'multiple-int' | 'string' | 'boolean';

type FilterConfig = {
    [Key in string]: { type: FilterParamType };
};
type ParsedFilters<Config extends FilterConfig> = {
...
type StringKeys<Config extends FilterConfig> = {
    [Key in keyof Config]: Config[Key]['type'] extends 'string' ? Key : never;
}[keyof Config];

export const useUrlFilters = <Config extends FilterConfig>(config: Config) => {
